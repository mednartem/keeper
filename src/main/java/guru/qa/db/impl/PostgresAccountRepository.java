...
public class PostgresAccountRepository implements AccountRepository {
    ...
    public void addAccount(AccountEntity account) {
        template.update("INSERT INTO account (name , value) values (?, ?)", account.getName(), account.getBalance());
    }

    @Override
    public void updateAccount(AccountEntity account) {
        template.update("UPDATE  account SET value = ? WHERE name = ?", account.getBalance(), account.getName());
    }
}
